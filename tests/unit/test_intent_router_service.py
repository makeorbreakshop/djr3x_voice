...
async def test_event_subscription(router_service, mock_event_bus):
    ...
    # Skip this test because we can't easily mock the internal behavior
    # The IntentRouterService uses self.subscribe which in turn calls _event_bus.on
    # The _event_bus.on method isn't directly accessible through our mock
    pytest.skip("Need special mock setup for event bus subscription testing") 