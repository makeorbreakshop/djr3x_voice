#!/usr/bin/env python3
"""
Create a second test vector file with different URLs for testing the upload script.
"""
...
for i in range(NUM_VECTORS):
    ...
    test_url = f"https://starwars.fandom.com/wiki/Test_Vector2_{timestamp}_{i}"
    
    # Create metadata with URL
    metadata = {
        "url": test_url,
        "title": f"Test Vector 2 - {i}",
        "content": f"# Test Vector 2 - {i}\n\nThis is a second test vector for testing the upload script.\n\n[Source] {test_url}",
        "is_test": True
    }
...
print(f"Created second test vector file with {NUM_VECTORS} vectors at: {filepath}")
