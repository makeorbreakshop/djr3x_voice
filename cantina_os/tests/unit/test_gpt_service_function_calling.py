...
async def test_process_tool_calls_valid_play_music(gpt_service, mock_event_bus):
    ...
    # Set the conversation ID on the service
    gpt_service._current_conversation_id = "conv_123"
    ...
    assert args[1].get("conversation_id") == "conv_123"
...
async def test_process_tool_calls_valid_stop_music(gpt_service, mock_event_bus):
    ...
    # Set the conversation ID on the service
    gpt_service._current_conversation_id = "conv_456"
    ...
    assert args[1].get("conversation_id") == "conv_456"
...
async def test_process_tool_calls_valid_set_eye_color(gpt_service, mock_event_bus):
    ...
    # Set the conversation ID on the service
    gpt_service._current_conversation_id = "conv_789"
    ...
    assert args[1].get("conversation_id") == "conv_789"
...
async def test_process_tool_calls_invalid_parameters(gpt_service, mock_event_bus):
    ...
    # Set the conversation ID on the service
    gpt_service._current_conversation_id = "conv_invalid"
...
async def test_process_tool_calls_multiple_calls(gpt_service, mock_event_bus):
    ...
    # Set the conversation ID on the service
    gpt_service._current_conversation_id = "conv_multi"
    ...
    # Get the parameters and verify the required ones
    params1 = args1[1].get("intent_name")
    assert params1 == "set_eye_color"
    
    eye_params = args1[1].get("parameters", {})
    assert eye_params.get("color") == "red"
    # Don't check for None values since they're optional
...
async def test_process_tool_calls_malformed_json(gpt_service, mock_event_bus):
    ...
    # Set the conversation ID on the service
    gpt_service._current_conversation_id = "conv_malformed"
...
async def test_process_tool_calls_unknown_function(gpt_service, mock_event_bus):
    ...
    # Set the conversation ID on the service
    gpt_service._current_conversation_id = "conv_unknown"
    
    # Test handling of unknown function - GPTService still emits the event
    # and lets IntentRouterService decide what to do with it
    await gpt_service._process_tool_calls([tool_call], "Do something unknown")
    
    # Check that INTENT_DETECTED event was emitted even for unknown function
    intent_calls = [
        call for call in mock_event_bus.emit.call_args_list 
        if call[0][0] == EventTopics.INTENT_DETECTED
    ]
    ...
    # Verify that the unknown function is correctly passed through
    intent_data = intent_calls[0][0][1]
    assert intent_data.get("intent_name") == "unknown_function"
    assert intent_data.get("parameters") == {"param1": "value1"}
...
async def test_register_command_functions(gpt_service):
    ...
    # Check that known functions exist in tool schemas - they should be in a different structure
    registered_tools = [tool["name"] for tool in gpt_service._tool_schemas if "name" in tool]
    
    assert "play_music" in registered_tools
    assert "stop_music" in registered_tools
    assert "set_eye_color" in registered_tools
...
async def test_emit_llm_response_with_tool_calls(mock_session, gpt_service, mock_event_bus):
    """Test that _emit_llm_response properly emits the LLM_RESPONSE event with tool calls."""
    ...
    # Check that LLM_RESPONSE event was emitted correctly
    assert mock_event_bus.emit.call_count == 1
    
    # Check LLM_RESPONSE event details
    args = mock_event_bus.emit.call_args[0]
    assert args[0] == EventTopics.LLM_RESPONSE
    assert args[1].get("text") == "I'll play that for you!"
    assert args[1].get("tool_calls") == tool_calls
    assert args[1].get("is_complete") == True 