...
class WebBridgeService(BaseService):
    ...
    def __init__(self, event_bus, config: Dict[str, Any] = None):
        ...
        self._throttle_limits = {
            'high_frequency': {'max_per_second': 10, 'events': self._high_frequency_events},
    ...
    async def _start(self) -> None:
        ...
        self._event_bus.emit(EventTopics.SERVICE_STATUS_REQUEST, {
            'source': 'web_bridge',
            'timestamp': datetime.now().isoformat()
        })
        
        self._status = ServiceStatus.RUNNING
        await self._emit_status(ServiceStatus.RUNNING, "Web Bridge Service started successfully")
    ...
    def _add_socketio_handlers(self) -> None:
        ...
        async def connect(sid, environ):
            ...
            await self._sio.emit('system_status', {
                'cantina_os_connected': True,
                'services': self._get_service_status(),
                'timestamp': datetime.now().isoformat()
            }, room=sid)
        ...
        async def subscribe_events(sid, data):
            ...
            if sid in self._dashboard_clients:
                ...
                self._dashboard_clients[sid]['filter_level'] = data.get('filter_level', 'all')
        ...
        async def voice_command(sid, data):
            ...
            try:
                ...
                await self._sio.emit('error', {'message': f'Failed to execute voice command: {e}'}, room=sid)
        ...
        async def music_command(sid, data):
            ...
            try:
                ...
                if action == 'play':
                    self._event_bus.emit(EventTopics.MUSIC_COMMAND, {
                        'action': 'play',
                    ...
                    self._event_bus.emit(EventTopics.DJ_NEXT_TRACK, {
                        'source': 'web_dashboard'
                    })
            except Exception as e:
                logger.error(f"Error forwarding music command: {e}")
                await self._sio.emit('error', {'message': f'Failed to execute music command: {e}'}, room=sid)
    ...
    def _subscribe_to_events(self) -> None:
        ...
        self._event_bus.on(EventTopics.SERVICE_STATUS_UPDATE, self._handle_service_status_update)
        self._event_bus.on(EventTopics.TRANSCRIPTION_FINAL, self._handle_transcription_final)
        self._event_bus.on(EventTopics.TRANSCRIPTION_INTERIM, self._handle_transcription_interim)
        self._event_bus.on(EventTopics.VOICE_LISTENING_STARTED, self._handle_voice_listening_started)
        self._event_bus.on(EventTopics.VOICE_LISTENING_STOPPED, self._handle_voice_listening_stopped)
        self._event_bus.on(EventTopics.MUSIC_PLAYBACK_STARTED, self._handle_music_playback_started)
        self._event_bus.on(EventTopics.MUSIC_PLAYBACK_STOPPED, self._handle_music_playback_stopped)
    ...
    async def _broadcast_event_to_dashboard(self, event_topic: str, data: Dict[str, Any], event_name: str = None):
        ...
        for sid in self._dashboard_clients.keys():
            try:
                await self._sio.emit(event_name or 'cantina_event', event_data, room=sid)
            except Exception as e:
