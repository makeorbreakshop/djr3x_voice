...
class DeepgramDirectMicService(BaseService):
    ...
    def __init__(
        ...
        # NOTE: SDK 4.x compatible - endpointing/filler_words require SDK 5.x (breaking changes)
    ...
    async def _start(self) -> None:
        ...
        try:
            ...
            # OPTIMIZATION: Start WebSocket connection once at service startup
            # This keeps the connection persistent across multiple recording sessions
            # instead of reconnecting on every mouse click (saves ~5 seconds per interaction)
            # Deepgram WebSocket timeout is 60 minutes, NOT 10 seconds (that was a misread)
            self._dg_connection.start(self._dg_options)
            if self._logger:
                self._logger.info("Deepgram WebSocket connection opened and ready for streaming")
    ...
    async def _handle_mic_recording_stop(self, event: Dict[str, Any]) -> None:
        ...
        try:
            # OPTIMIZATION FIX: Stop ONLY the microphone, NOT the WebSocket connection!
            # The 5-second delay was caused by closing and reopening the WebSocket on every click.
            # Now we keep the WebSocket open and just stop the microphone stream.
            if self._microphone:
                self._microphone.finish()  # Synchronous - stops mic only
                self._microphone = None
                if self._logger:
                    self._logger.info("Microphone.finish() called (WebSocket stays open)")

            # DO NOT call self._dg_connection.finish() here!
            # The WebSocket connection stays open for the next recording session.
            # It will only be closed on service shutdown (_stop method).

            # Short delay to allow any in-flight audio data to be transcribed
            # Reduced from 250ms to 50ms since we're not waiting for connection teardown
            await asyncio.sleep(0.05)  # 50ms for final transcript buffer flush
    ...
    async def _start_listening(self) -> None:
        ...
        try:
            ...
            # OPTIMIZATION: WebSocket connection is already open from _start()
            # Just create and start the microphone to stream audio
            # (No need to reconnect WebSocket on every recording session)
            self._microphone = Microphone(self._dg_connection.send)
            self._microphone.start()

            self._is_listening = True
            if self._logger:
                self._logger.info("Started microphone streaming to Deepgram (connection already open)")
    ...
    async def _stop_listening(self) -> None:
        ...
        try:
            ...
            # OPTIMIZATION: Keep WebSocket connection open, don't close it here
            # It will be closed on service shutdown (_stop method)
